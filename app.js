...
function Planet(props){
  return(
    <div className="card">
      <div>
        <img src={props.planet.url} alt={props.planet.name}></img>
      </div>
      <h2>{props.planet.name}</h2>
      <p>{props.planet.desc}</p>
      <h3>Planet Profile</h3>
      <ul>
        <li><strong>Diameter:</strong> {props.planet.diameter} mi</li>
        <li><strong>Moons:</strong> {props.planet.moons}</li>
      </ul>
    </div>
  );
}
